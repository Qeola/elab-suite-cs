...
const DepartmentModal = ({loading, setLoading}: {loading: boolean, setLoading: (e:boolean) => void }) => {
    ...
    <div className='space-y-6'>
        <Formik
            ...
            <Form className="mt-6">
              {/* Email */}
              <div className="mb-4">
                    <Label htmlFor="email" value="Department Name" className="mb-2 block" />
                    <Field id="name" name="name" type="text" className="form-control w-full" sizing='lg' as={TextInput} />
                    <ErrorMessage name="name" component="div" className="text-red-500 text-sm mt-1" />
                  </div>
                  <div className="mt-6">
          {loading ? <AuthLoadingButton>Onboarding</AuthLoadingButton> : <AuthButton>Onboarding</AuthButton>}
          </div>
            </Form>
              )}
        </Formik>
